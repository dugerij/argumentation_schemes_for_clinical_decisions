...
RELATION_STRING_RE = re.compile(
    r"^(?P<source>\S+)\s+->\s+(?P<label>[A-Z_]+)\s+->\s+(?P<target>\S+)$"
)
...
def _index_mode(index: Any) -> str | None:
    mode = getattr(index, "_index_mode", None)
...
def _node_embedding(node: Any) -> list[float] | None:
...
def _implicit_chunk_matches(
    ...
    query_embedding = _embed_text(embed_model, query)
    top_matches: list[tuple[float, Any]] = []

    for node in graph_nodes.values():
        if getattr(node, "label", None) != "text_chunk":
            continue
        node_embedding = _node_embedding(node)
        if node_embedding is None:
            continue
        similarity = _cosine_similarity(query_embedding, node_embedding)
        if similarity is None:
            continue
        item = (similarity, node)
        if len(top_matches) < similarity_top_k:
            heapq.heappush(top_matches, item)
        elif similarity > top_matches[0][0]:
            heapq.heapreplace(top_matches, item)

    ranked = sorted(top_matches, key=lambda item: item[0], reverse=True)
    return [(node, score) for score, node in ranked], query_embedding


def _implicit_context_parts(matches: list[tuple[Any, float]], graph_store: Any | None) -> list[str]:
    parts: list[str] = []
    seen: set[str] = set()
...
def _implicit_response_prompt(query: str, context: str) -> str:
    return (
        "You are answering a clinical retrieval query using only the provided clinical note excerpts.\n"
...
async def query_index_with_diagnostics(
    ...
    resolved_llm = llm or index_llm or build_llm()
    graph_store = _graph_store(index)

    retrieval_scores: list[float] = []
    cosine_similarities: list[float] = []

    if _index_mode(index) == "implicit":
        resolved_embed_model = embed_model or getattr(index, "_embed_model", None) or build_embed_model()
        matches, _query_embedding = _implicit_chunk_matches(
            index=index,
            query=query,
            similarity_top_k=similarity_top_k,
            embed_model=resolved_embed_model,
        )
        context_parts = _implicit_context_parts(matches, graph_store=graph_store)
        context = "\n\n".join(context_parts)
        retrieval_scores = [score for _node, score in matches]
        cosine_similarities = list(retrieval_scores)

        if context:
            response = await asyncio.to_thread(
                _llm_complete_text,
                resolved_llm,
                _implicit_response_prompt(query, context),
            )
        else:
            response = "No relevant clinical note excerpts were retrieved for the query."
    else:
